    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            subjectIds.amap { (subjectId, language) ->
                try {
                    var gotStreams = false

                    // ── RESOURCE_PATH (parthmax2/movie-box v3) ──
                    val resourceUrl = "$mainUrl/wefeed-mobile-bff/subject-api/resource?subjectId=$subjectId&se=$season&ep=$episode"
                    val resXClientToken = generateXClientToken()
                    val resXTrSignature = generateXTrSignature("GET", "application/json", "application/json", resourceUrl)
                    val resHeaders = mapOf(
                        "Authorization" to "Bearer $token",
                        "user-agent" to "com.community.oneroom/50020088 (Linux; U; Android 13; en_US; $brand; Build/TQ3A.230901.001; Cronet/145.0.7582.0)",
                        "accept" to "application/json",
                        "content-type" to "application/json",
                        "connection" to "keep-alive",
                        "x-client-token" to resXClientToken,
                        "x-tr-signature" to resXTrSignature,
                        "x-client-info" to """{"package_name":"com.community.oneroom","version_name":"3.0.13.0325.03","version_code":50020088,"os":"android","os_version":"13","install_ch":"ps","device_id":"$deviceId","install_store":"ps","gaid":"1b2212c1-dadf-43c3-a0c8-bd6ce48ae22d","brand":"$model","model":"$brand","system_language":"en","net":"NETWORK_WIFI","region":"US","timezone":"Asia/Calcutta","sp_code":"","X-Play-Mode":"1","X-Idle-Data":"1","X-Family-Mode":"0","X-Content-Mode":"0"}""".trimIndent(),
                        "x-client-status" to "0"
                    )
                    val resResponse = app.get(resourceUrl, headers = resHeaders)
                    if (resResponse.code == 200) {
                        val resRoot = mapper.readTree(resResponse.text)
                        val rd = resRoot["data"]
                        val resources = rd?.get("resources") ?: rd?.get("streams")
                        if (resources != null && resources.isArray && resources.size() > 0) {
                            for (res in resources) {
                                val resLink = res["resourceLink"]?.asText() ?: res["url"]?.asText() ?: continue
                                val resId = res["resourceId"]?.asText() ?: res["id"]?.asText() ?: "$subjectId|$season|$episode"
                                val resQuality = res["resolution"]?.asInt() ?: 0
                                callback.invoke(
                                    newExtractorLink(
                                        source = "$name ${language.replace("dub","Audio")}",
                                        name = "$name ${resQuality}p (${language.replace("dub","Audio")})",
                                        url = resLink,
                                        type = ExtractorLinkType.VIDEO
                                    ) {
                                        this.headers = mapOf("Referer" to mainUrl)
                                        if (resQuality > 0) this.quality = resQuality
                                    }
                                )
                                gotStreams = true
                            }
